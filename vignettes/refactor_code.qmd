...
```{r, paged.print=FALSE}
precalc_res <- precalculate_results2(spec, sce, dbfile = "~/Downloads/result_cache/kang.duckdb")
```

```{r}
run_shinyTreelabel(spec, col_data = as_tibble(colData(sce)), precalc_results = "~/Downloads/result_cache/kang.duckdb")
```


```{r, paged.print=FALSE}
run_shinyTreelabel(spec, sce, precalc_results = precalc_res)
```
